...
#'
#' @param model Should be a string of model name
#'
...
ezr.h2o_xgb_importance_aggregated  = function(model){
  ...
  if(as.character(model@algorithm) !='xgboost'){
    ...
    result = model_varimp %>% mutate(
      scaled_importance = percentage / max_percent,
      relative_importance = (max_percent / min_percent) * scaled_importance
    ) %>% dplyr::arrange(desc(percentage))  %>% mutate(
      rank = dense_rank(desc(percentage)),
      type = ifelse(n > 1, paste0('cat_n',n),'numeric')
    )
  }
